...
class ObservationProcessor:
    ...
    def get_speed(observation):
        ...
        cropped_state = gray_state[88:95, 12:13]
    ...
    def get_left_steering(observation):
        gray_state = cv2.cvtColor(observation, cv2.COLOR_RGB2GRAY)
        cropped_state = gray_state[89:90, 38:47]
        #observation_resized = cv2.resize(cropped_state, (40 * cropped_state.shape[1], 40 * cropped_state.shape[0]))
        return ObservationProcessor._threshold_and_sum(cropped_state)
    ...
    def get_right_steering(observation):
        gray_state = cv2.cvtColor(observation, cv2.COLOR_RGB2GRAY)
        cropped_state = gray_state[89:90, 48:57]
        #observation_resized = cv2.resize(cropped_state, (40 * cropped_state.shape[1], 40 * cropped_state.shape[0]))
        return ObservationProcessor._threshold_and_sum(cropped_state)
    ...
    def get_right_ABS(observation):
        gray_state = cv2.cvtColor(observation, cv2.COLOR_RGB2GRAY)
        cropped_state = gray_state[89:93, 22:23]
        #observation_resized = cv2.resize(cropped_state, (40 * cropped_state.shape[1], 40 * cropped_state.shape[0]))
        return ObservationProcessor._threshold_and_sum(cropped_state)
    ...
    def get_left_ABS(observation):
        gray_state = cv2.cvtColor(observation, cv2.COLOR_RGB2GRAY)
        cropped_state = gray_state[89:93, 18:19]
        #observation_resized = cv2.resize(cropped_state, (40 * cropped_state.shape[1], 40 * cropped_state.shape[0]))
        return ObservationProcessor._threshold_and_sum(cropped_state)
    ...
    def get_right_gyro(observation):
        gray_state = cv2.cvtColor(observation, cv2.COLOR_RGB2GRAY)
        cropped_state = gray_state[89:90, 72:76]
        #observation_resized = cv2.resize(cropped_state, (40 * cropped_state.shape[1], 40 * cropped_state.shape[0]))
        return ObservationProcessor._threshold_and_sum(cropped_state)
    ...
    def get_left_gyro(observation):
        gray_state = cv2.cvtColor(observation, cv2.COLOR_RGB2GRAY)
        cropped_state = gray_state[89:90, 68:72]
        #observation_resized = cv2.resize(cropped_state, (40 * cropped_state.shape[1], 40 * cropped_state.shape[0]))
        return ObservationProcessor._threshold_and_sum(cropped_state)
    
    @staticmethod
    def get_vision(observation):
        block_vision = observation[:-12, :, :]
...
class DQNAgent:
    def __init__(self, input_size, output_size, epsilon_decay=0.995, min_epsilon=0.01, lr=0.1):
...
state_size = 8072  # Change this to the actual size
action_size = 5   # Change this to the actual size
epsilon = 0.1

pathname = 'DQN_agent_4'
# Initialize DQNAgent
...
for episode in range(episodes):
    ...
    for timestep in range(timesteps):
        ...
        #gray_vision_sum = sum(vision_list[:8064])
        #print(gray_vision_sum)
